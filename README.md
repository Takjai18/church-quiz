...
新開場喺主持台 **後台** 可以設定今次用幾種題型、每種幾多題、每題幾多分（超過 3 題可自訂分數）。改題同答案喺題庫專頁。

題庫專頁：[/bank](https://church-quiz.deciduous-crayfish.workers.dev/bank)

- **入題（備用）**：題目存入資料庫，可按類別同 10／30／50 分篩選。主持可以加新類別（詩歌、歷史、常識等）
- **揀題（出賽）**：每個類別揀齊 10／30／50，再套用到主持台。新類別揀齊三格就會上棋盤

韓劇題可以上傳劇照。主持台後台都有「題庫」連結。
